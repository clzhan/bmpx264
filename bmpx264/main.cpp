...
int main(int argc, char*argv[])
{
	...
	for (int i = 1; i < argc ; ++i)
	{
		...
		printf(" RGB2YUV \n");
		memset(yuv.Y, 0, bmp.height * bmp.width);
		memset(yuv.U, 0, ((bmp.height * bmp.width) / 4));
		memset(yuv.V, 0, ((bmp.height * bmp.width) / 4));
		//rgb2yuv
		
		//RGB2YUV(bmp.width, bmp.height, bmp.rgbData, yuv.Y, yuv.U, yuv.V);
		//RGB2YUV_2(bmp.rgbData, bmp.width, bmp.height, yuv.Y, yuv.U, yuv.V);

		RGBTOYUV420P(bmp.rgbData, bmp.width, bmp.height, bmp.width, bmp.height, yuv.Y, yuv.U, yuv.V);

		//WriteYuv(yuvfile, bmp, yuv);
		//fclose(yuvfile);
		
		printf("encode yuv\n");
		Encode_YUV(yuv.Y, yuv.U, yuv.V, bmp.width, bmp.height, &H264Data, frameLength);
